...
emoji: "📸"
type: "tech"
topics: ["shell", "zsh", "claudecode", "claude", "cli"]
published: true
---
...
:::message
この記事はClaude Codeの内部実装を調べてみた記事です。実践的なノウハウやTipsは含まれていません。仕組みに興味がある方は気楽に読んでください。
:::
...
```mermaid
graph TD
    A[親シェル] -->|fork + exec| B[サブシェル<br/>新しいプロセス<br/>変数はサブシェル内のみ]
```
...
```mermaid
graph TD
    A[現在のシェル<br/>同じプロセス内<br/>変数・関数が残る]
```
