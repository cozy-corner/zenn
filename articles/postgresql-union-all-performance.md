...
図書館に検索用PCにキーワードを入力すると、著者名・タイトル・出版社名のどれかにマッチする本がずらっと表示されるような機能がありますよね。
...
素直な書き方です。
私も小規模なシステムでパフォーマンス要件が厳しくない場合は、まずはこう書きます。
...
- 著者だけで検索する場合も出版社テーブルまで結合するため、無駄な処理が発生
...
**UNION ALL + DISTINCT:**
- 各サブクエリの結果をUNION ALLで統合し、最後に1回だけ重複除去

### 4. 並列実行

このケースでは**Parallel Append**（並列実行）が発動しました。

複数のサブクエリを並列に実行できたため、これが最速化の主な要因と考えられます。
...
以下の条件を満たす場合、UNION ALL + DISTINCTを検討する価値がありそうです。
...
まずOR条件で実装し、パフォーマンス問題が発生したら`EXPLAIN ANALYZE`で比較して選択するのが良いと思います。

## まとめ

OR条件とUNION ALL + DISTINCTでは、実行計画が大きく異なることがわかりました。今回のケースでは並列実行（Parallel Append）の発動が決め手でした。

今回は別の観点で調査をしていた過程で Claude Code が提案した方法について、実際に検証しながら、深掘ってみました。

AI は不正確な情報を出すこともありますが、自分の知識にないアイデアを提案してくれるのがありがたいです。

時には脱線しながら学びに繋げるのも楽しいですね。
